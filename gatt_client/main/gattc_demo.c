...
void init_display() {
    ...
    ssd1306_display_text(&dev, 0, "BLE Client Ready", 16, false);
}
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_CONNECT_EVT:{
        ...
        // --- STABILIZATION DELAY ---
        // Wait 500ms to allow the BLE radio link to reach a steady state
            ...
            if (count > 0){
                ...
                if (!descr_elem_result){
                    ...
                    ret_status = esp_ble_gattc_get_descr_by_char_handle( gattc_if,
                                                                         ...
                                                                         &count);
                    if (ret_status != ESP_GATT_OK){
                        ESP_LOGE(GATTC_TAG, "esp_ble_gattc_get_descr_by_char_handle error: 0x%x", ret_status);
                    ...
                    if (count > 0 && descr_elem_result[0].uuid.len == ESP_UUID_LEN_16 && descr_elem_result[0].uuid.uuid.uuid16 == ESP_GATT_UUID_CHAR_CLIENT_CONFIG){
                        ...
                        // --- ADDED LOGGING ---
                        ESP_LOGI(GATTC_TAG, "Writing 0x01 to CCCD Descriptor Handle: %d", descr_elem_result[0].handle);
                        // ---------------------
                ...
                ESP_LOGE(GATTC_TAG, "descr not found"); // Fixed typo from 'decsr' to 'descr'
            }
...
void app_main(void)
{
    ...
    // Note: Make sure the width/height match your specific OLED (e.g., 128x64)
    ssd1306_init(&dev, 128, 64);
    ssd1306_clear_screen(&dev, false);
    ssd1306_display_text(&dev, 0, "BLE Connected!", 14, false);
    ...
    // --- ADD THE DELAY HERE ---
    ESP_LOGI(GATTC_TAG, "Waiting for server to stabilize...");
